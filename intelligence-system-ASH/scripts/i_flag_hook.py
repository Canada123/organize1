...
def parse_index_flag(prompt):
    """Parse -i or -ic flag with optional size and depth.

    Flag syntax: -i[size][d[depth]]
    Examples:
      -i          -> default size, unlimited depth
      -i50        -> 50k tokens, unlimited depth
      -i50d       -> 50k tokens, explicit unlimited depth
      -i50d10     -> 50k tokens, depth 10
      -id5        -> default size, depth 5
    """
    # Pattern matches -i[c][number][d[depth]]
    # Groups: (c?)(size?)d?(depth?)
    match = re.search(r'-i(c?)(\d+)?(d(\d+)?)?(?:\s|$)', prompt)
    ...
    # Parse depth
    tree_depth = None  # Default unlimited
    if match.group(3):  # Has 'd' flag
        if match.group(4):  # Has number after 'd'
            tree_depth = int(match.group(4))
            print(f"📏 Tree depth: {tree_depth}", file=sys.stderr)
        else:
            # 'd' without number = explicit unlimited
            tree_depth = None
            print(f"📏 Tree depth: unlimited", file=sys.stderr)
    ...
    # Clean prompt (remove flag including depth)
    cleaned_prompt = re.sub(r'-ic?\d*(d\d*)?\s*', '', prompt).strip()

    return size_k, clipboard_mode, tree_depth, cleaned_prompt
...
def generate_index_at_size(project_root, target_size_k, tree_depth=None, is_clipboard_mode=False):
    """Generate index at specific token size and tree depth.

    Args:
        project_root: Root directory to index
        target_size_k: Target size in k tokens
        tree_depth: Maximum tree depth (None = unlimited, int = specific limit)
        is_clipboard_mode: Whether in clipboard export mode
    """
    depth_msg = f"unlimited depth" if tree_depth is None else f"depth {tree_depth}"
    print(f"🎯 Generating {target_size_k}k token index with {depth_msg}...", file=sys.stderr)
    
    # Find indexer script
    local_indexer = Path(__file__).parent / 'project_index.py'
    system_indexer = Path.home() / '.claude-intelligence-system' / 'scripts' / 'project_index.py'
        ...
        if result.returncode == 0:
            ...
            if index_path.exists():
                ...
                metadata_update = {
                    ...
                    'tree_depth': tree_depth if tree_depth is not None else 'unlimited',
...
def main():
    ...
    try:
        ...
        # Parse flag
        size_k, clipboard_mode, tree_depth, cleaned_prompt = parse_index_flag(prompt)
        ...
        if should_regen:
            ...
            if not generate_index_at_size(project_root, size_k, tree_depth, clipboard_mode):
