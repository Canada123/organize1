...
from index_utils import (
    ...
    extract_shell_signatures, extract_csharp_signatures, extract_markdown_structure, 
    infer_file_purpose, infer_directory_purpose, get_language_name, should_index_file,
    detect_tools_from_taxonomy,  # ADD THIS
    detect_algorithms_from_taxonomy,  # ADD THIS
    detect_game_dev_tools  # ADD THIS for game dev taxonomy
)
try:
    ...
    print("âš ï¸  tree-sitter-languages not installed. Install with: pip install tree-sitter-languages")

# Limits to keep it fast and simple
MAX_FILES = 10000
MAX_INDEX_SIZE = 5 * 1024 * 1024   # 5MB instead of 1MB
DEFAULT_TREE_DEPTH = None  # Default unlimited tree depth (use int to limit, or set via TREE_DEPTH env var / -id flag)

def generate_tree_structure(root_path: Path, max_depth: int = None) -> List[str]:
    """Generate a compact ASCII tree representation of the directory structure.  
  
    Args:  
        root_path: Root directory to generate tree from  
        max_depth: Maximum tree depth (None = unlimited, int = specific depth limit)  
    """
    tree_lines = []
...
def build_index(root_dir: str, tree_depth: int = None) -> Tuple[Dict, int]:
    """Build the index by collecting and processing all files"""
      
    # Initialize root path (4 spaces indent)
    root = Path(root_dir)
      
    # Initialize index dictionary (4 spaces indent)
    index = {
        'indexed_at': datetime.now().isoformat(),
        'root': str(root),
        'project_structure': {'type': 'tree', 'root': '.', 'tree': []},
        'documentation_map': {},
        'directory_purposes': {},
        'stats': {
            'total_files': 0,
    ...
    # Initialize counters (4 spaces indent)
    file_count = 0
    dir_count = 0
    skipped_count = 0
    directory_files = {}
      
    # Try to use git ls-files for better performance (4 spaces indent)
    print("🔍 Indexing files...")
    from index_utils import get_git_files
    git_files = get_git_files(root)
      
    # File discovery (4 spaces indent)
    if git_files is not None:
        print(f"   Using git ls-files (found {len(git_files)} files)")
        files_to_process = git_files
          
        # Count directories from git files (8 spaces - inside if block)
        seen_dirs = set()
        ...
        # Fallback to manual file discovery (8 spaces - inside else block)
    ...
    # Process files (4 spaces - AFTER if/else, not inside it)
    for file_path in files_to_process:
        ...
        # Track files in their directories (8 spaces - inside loop)
        parent_dir = file_path.parent
        if parent_dir in directory_files:
            directory_files[parent_dir].append(file_path.name)
          
        # Get relative path and language (8 spaces - inside loop)
        rel_path = file_path.relative_to(root)
          
        # Handle markdown files specially (8 spaces - inside loop)
        ...
        # Handle code files (8 spaces - inside loop)
        language = get_language_name(file_path.suffix)
          
        # Base info for all files (8 spaces - inside loop)
        file_info = {
            'language': language,
            'parsed': False
        }
          
        # Add file purpose if we can infer it (8 spaces - inside loop)
        ...
        if file_path.suffix in PARSEABLE_LANGUAGES:
            try:
                ...
                # Extract based on language (12 spaces - inside try)
                if file_path.suffix == '.py':
                    ...
                    extracted = extract_csharp_signatures(content)
                else:
                    extracted = {'functions': {}, 'classes': {}}
                  
                # Only add if we found something (12 spaces)
                if extracted['functions'] or extracted['classes']:
                    file_info.update(extracted)
                    file_info['parsed'] = True
                  
                # NEW: Detect tools (12 spaces)
                tool_hits = detect_tools_from_taxonomy(
                    file_path=file_path,
                    language=language,
                ...
                if tool_hits:
                    file_info['tools'] = tool_hits
                  
                # NEW: Detect algorithms (12 spaces)
                algo_hits = detect_algorithms_from_taxonomy(
                    file_path=file_path,
                    language=language,
                    content_snippet=content[:4000]
                )
                if algo_hits:
                    file_info['algorithms'] = algo_hits
                  
                # NEW: Detect game dev tools (12 spaces)
                game_dev_hits = detect_game_dev_tools(
                    file_path=file_path,
                    language=language,
                    imports=extracted.get('imports', []),
                    content_snippet=content[:4000]
                ...
                if game_dev_hits:
                    file_info['game_dev'] = game_dev_hits
                  
                # Update stats (12 spaces)
                ...
                # Parse error - just list the file (12 spaces)
        ...
        # Progress indicator (8 spaces - inside loop)
        if file_count % 100 == 0:
            print(f"  Indexed {file_count} files...")
      
    # Infer directory purposes (4 spaces - after loop)
    ...
    # Generate directory tree AT THE END
    print("ðŸ“Š Building directory tree...")
    index['project_structure']['tree'] = generate_tree_structure(root, tree_depth)
...
def convert_to_enhanced_dense_format(index: Dict) -> Dict:
    ...
    dense = {
        ...
        'root': index.get('root', '.'),
        'tree': index.get('project_structure', {}).get('tree', []),
...
def compress_if_needed(dense_index: Dict, target_size: int = MAX_INDEX_SIZE) -> Dict:
    ...
    # Allow 10% overage before compressing
    ...
    if dense_index.get('f'):
        ...
        print(f"  âœ… Kept {len(dense_index['f'])} most important files")
      
    final_size = len(json.dumps(dense_index, separators=(',', ':')))
    compression_ratio = (1 - final_size / len(index_json)) * 100
    print(f"  ðŸ“Š Final size: {final_size} bytes ({compression_ratio:.1f}% reduction)")
...
def main():
    ...
    # Check for tree depth from environment
    tree_depth_env = os.getenv('TREE_DEPTH')
    if tree_depth_env:
        if tree_depth_env.lower() == 'unlimited':
            tree_depth_int = None
            print(f"   Tree depth: unlimited")
        else:
            tree_depth_int = int(tree_depth_env)
            print(f"   Tree depth: {tree_depth_int}")
    else:
        tree_depth_int = DEFAULT_TREE_DEPTH
        print(f"   Tree depth: {tree_depth_int} (default)")
    
    print("   Analyzing project structure and documentation...")

    # Build index for current directory
    index, skipped_count = build_index('.', tree_depth_int)
